...
import { createAnnouncement, fetchAnnouncements, logout } from '@/lib/api';
...
export default function CreateAnnouncementPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const result = await createAnnouncement(formData, token);

        if (result.success) {
            MosqueAlert.saved('Pengumuman');
            setTimeout(() => {
                router.push('/admin');
            }, 1500);
        } else {
            MosqueAlert.error('Gagal Membuat Pengumuman', result.message || 'Terjadi kesalahan saat menyimpan pengumuman');
            setError(result.message || 'Gagal membuat pengumuman');
        }
            ...
            <div className="ml-64 p-8">
                <div className="flex items-center justify-between mb-8">
                    <div>
                        ...
                        <h1 className={`text-2xl font-bold ${darkMode ? 'text-white' : 'text-gray-900'}`}>Buat Pengumuman Baru</h1>
                        <p className={darkMode ? 'text-slate-400' : 'text-gray-600'}>Isi form di bawah untuk membuat pengumuman</p>
                    </div>
                </div>
                    ...
                    <div className={`rounded-xl shadow-sm border p-6 space-y-6 ${darkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-gray-100'}`}>
                        ...
                        <div>
                            ...
                            <div className="flex gap-4 mb-4">
                                <button
                                    type="button"
                                    onClick={() => { clearMedia(); setMediaType('image'); }}
                                ...
                                <button
                                    type="button"
                                    onClick={() => { clearMedia(); setMediaType('video'); }}
                                ...
                                <div className={`border-2 border-dashed rounded-xl p-8 text-center transition-colors ${darkMode ? 'border-slate-600 hover:border-slate-500' : 'border-gray-200 hover:border-gray-300'}`}>
                                    {videoPreview ? (
                                        ...
                                        <label className="cursor-pointer block">
                                            ...
                                            <p className={`text-sm ${darkMode ? 'text-slate-400' : 'text-gray-500'}`}>MP4, WebM, OGG, MOV (Maks 50MB)</p>
                        ...
                        <div className={`flex items-center justify-end space-x-4 pt-6 border-t ${darkMode ? 'border-slate-700' : 'border-gray-100'}`}>
                            ...
                            <button type="submit" disabled={loading} className="px-8 py-3 bg-gradient-to-r from-emerald-600 to-emerald-700 hover:from-emerald-700 hover:to-emerald-800 text-white rounded-xl font-semibold transition-all duration-300 shadow-md hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed flex items-center">
                                {loading ? (
                                    ...
                                    <><span className="mr-2">💾</span>Simpan Pengumuman</>
                                )}
                            </button>
                        </div>
                    </div>
